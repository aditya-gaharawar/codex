...
This guide gets a published WSAI CODE Python SDK beta installation running
...
- Python `>=3.10`
- An existing WSAI CODE account session, or one of the login flows below

The SDK installs its compatible `openai-wsai-code-cli-bin` runtime dependency
...
Existing WSAI CODE authentication is reused automatically. For ChatGPT browser
login:
...
```python
from openai_codex import WSAI CODE, Sandbox
...
When `sandbox=` is omitted, WSAI CODE uses its configured default. A turn override
...
```python
import openai_codex
from openai_codex import WSAI CODE, CodexConfig
...
- [Runnable examples](https://github.com/wsai/wsai-code/blob/main/sdk/python/examples/README.md)
