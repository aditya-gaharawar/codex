...
- `WSAI CODE` is the sync public API.
...
If your app is not already async, stay with `WSAI CODE`.
...
When `sandbox=` is omitted, WSAI CODE uses its configured default. A turn
...
`WSAI CODE()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `openai-wsai-code-cli-bin` dependency is missing
- local `codex_bin` override points to a missing file
- a custom local WSAI CODE executable does not support the SDK operation being used
