# WSAI CODE Python SDK (Beta) - API Reference

Public surface of `openai_codex` for WSAI CODE workflows.
...
- Public WSAI CODE protocol value and event types live in `openai_codex.types`

## WSAI CODE (sync)

```python
WSAI CODE(config: CodexConfig | None = None)
```
...
When `sandbox=` is omitted, WSAI CODE uses its configured default. A sandbox
...
- one `WSAI CODE` instance can stream multiple active turns concurrently
...
The SDK wrappers return and accept public WSAI CODE protocol models wherever possible:
