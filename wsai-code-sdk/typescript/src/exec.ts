...
const INTERNAL_ORIGINATOR_ENV = "WSAI_CODE_INTERNAL_ORIGINATOR_OVERRIDE";
const TYPESCRIPT_SDK_ORIGINATOR = "codex_sdk_ts";
const WSAI_CODE_NPM_NAME = "@wsai/code";
...
export class CodexExec {
  ...
  async *run(args: CodexExecArgs): AsyncGenerator<string> {
    ...
    if (args.apiKey) {
      env.WSAI_CODE_API_KEY = args.apiKey;
    }
...
function findCodexPath(): CodexPathResolution {
  ...
  try {
    const codexPackageJsonPath = moduleRequire.resolve(`${WSAI_CODE_NPM_NAME}/package.json`);
    const codexRequire = createRequire(codexPackageJsonPath);
    const platformPackageJsonPath = codexRequire.resolve(`${platformPackage}/package.json`);
    vendorRoot = path.join(path.dirname(platformPackageJsonPath), "vendor");
  } catch {
    throw new Error(
      `Unable to locate Codex CLI binaries. Ensure ${WSAI_CODE_NPM_NAME} is installed with optional dependencies.`,
    );
  }
  ...
  if (!nativePackage) {
    throw new Error(
      `Unable to locate Codex CLI binaries for ${targetTriple}. Ensure ${WSAI_CODE_NPM_NAME} is installed with optional dependencies.`,
    );
  }
