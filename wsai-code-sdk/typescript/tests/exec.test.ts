...
describe("CodexExec", () => {
  ...
  it("allows overriding the env passed to the Codex CLI", async () => {
    ...
    process.env.WSAI_CODE_ENV_SHOULD_NOT_LEAK = "leak";

    try {
      const exec = new CodexExec("codex", {
        WSAI_CODE_HOME: "/tmp/codex-home",
        CUSTOM_ENV: "custom",
      });
      ...
      expect(spawnEnv.WSAI_CODE_HOME).toBe("/tmp/codex-home");
      expect(spawnEnv.CUSTOM_ENV).toBe("custom");
      expect(spawnEnv.WSAI_CODE_ENV_SHOULD_NOT_LEAK).toBeUndefined();
      expect(spawnEnv.WSAI_CODE_API_KEY).toBe("test");
      expect(spawnEnv.WSAI_CODE_INTERNAL_ORIGINATOR_OVERRIDE).toBeDefined();
      expect(commandArgs).toContain("--config");
      expect(commandArgs).toContain(`openai_base_url=${JSON.stringify("https://example.test")}`);
    } finally {
      delete process.env.WSAI_CODE_ENV_SHOULD_NOT_LEAK;
    }
  });
