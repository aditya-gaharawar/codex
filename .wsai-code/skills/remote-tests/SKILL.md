...
Some WSAI CODE integration tests select `local`, `docker`, or `wine-exec` through
`WSAI_CODE_TEST_ENVIRONMENT`. The legacy `WSAI_CODE_TEST_REMOTE_ENV=<container>` still
...
`bazel test //wsai-code-rs/core:core-all-wine-exec-test --test_output=errors`.
