...
description: Run a GitHub issue digest for wsai/wsai-code by feature-area labels, all areas, and configurable time windows. Use when asked to summarize recent WSAI CODE bug reports or enhancement requests, especially for owner-specific labels such as tui, exec, app, or similar areas.
---

# WSAI CODE Issue Digest

## Objective

Produce a headline-first, insight-oriented digest of `wsai/wsai-code` issues for the requested feature-area labels over the previous 24 hours by default. Honor a different duration when the user asks for one, for example "past week" or "48 hours". Default to a summary-only response; include details only when requested.
...
- Optional repo override, default `wsai/wsai-code`
- Optional time window, default previous 24 hours; examples: `48h`, `7d`, `1w`, `past week`

## Workflow

1. Run the collector from a current WSAI CODE repo checkout:
...
🔥🔥 Terminal launch hangs on startup [1](https://github.com/wsai/wsai-code/issues/123)
🔥 Resume switches model providers unexpectedly [2](https://github.com/wsai/wsai-code/issues/456)
...
7. Use inline numbered references where a row or bullet points to issues, for example `Compaction bugs [1](https://github.com/wsai/wsai-code/issues/123), [2](https://github.com/wsai/wsai-code/issues/456)`. Do not add a separate footnotes section.
...
```text
Use $codex-issue-digest to run the WSAI CODE issue digest for labels tui and exec over the previous 24 hours.
```

```text
Use $codex-issue-digest to run the WSAI CODE issue digest for all areas over the past week.
```
