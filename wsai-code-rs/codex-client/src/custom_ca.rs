...
//!
//! - read CA material from `WSAI_CODE_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
pub const WSAI_CODE_CA_CERT_ENV: &str = "WSAI_CODE_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set WSAI_CODE_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// CA handling before finally constructing the client. `WSAI_CODE_CA_CERTIFICATE` takes precedence
...
/// `WSAI_CODE_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
trait EnvSource {
    ...
    ///
    /// `WSAI_CODE_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Codex-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(WSAI_CODE_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: WSAI_CODE_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
mod tests {
    ...
    use super::WSAI_CODE_CA_CERT_ENV;
    use super::EnvSource;
    ...
    fn ca_path_prefers_codex_env() {
        let env = map_env(&[
            (WSAI_CODE_CA_CERT_ENV, "/tmp/codex.pem"),
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(WSAI_CODE_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);
