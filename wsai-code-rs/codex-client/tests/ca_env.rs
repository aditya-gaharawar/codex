...
const WSAI_CODE_CA_CERT_ENV: &str = "WSAI_CODE_CA_CERTIFICATE";
const PROBE_PROXY_ENV: &str = "WSAI_CODE_CUSTOM_CA_PROBE_PROXY";
const PROBE_TLS13_ENV: &str = "WSAI_CODE_CUSTOM_CA_PROBE_TLS13";
const PROBE_URL_ENV: &str = "WSAI_CODE_CUSTOM_CA_PROBE_URL";
...
fn probe_command() -> Command {
    ...
    cmd.env_remove(WSAI_CODE_CA_CERT_ENV);
...
fn uses_codex_ca_cert_env() {
    ...
    let output = run_probe(&[(WSAI_CODE_CA_CERT_ENV, cert_path.as_path())]);
...
fn prefers_codex_ca_cert_over_ssl_cert_file() {
    ...
    let output = run_probe(&[
        (WSAI_CODE_CA_CERT_ENV, cert_path.as_path()),
...
fn posts_to_tls13_server_using_custom_ca_bundle() {
    ...
    let output =
        run_probe_posting_to_tls13_server(&[(WSAI_CODE_CA_CERT_ENV, cert_path.as_path())], &server.url);
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("WSAI_CODE_CA_CERTIFICATE"));
