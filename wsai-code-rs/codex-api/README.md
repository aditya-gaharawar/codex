...
Typed clients for WSAI CODE/OpenAI APIs built on top of the generic transport in `wsai-code-client`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `codex-api` and `wsai-code-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
