...
//!   2. User-defined entries inside `~/.wsai-code/config.toml` under the `model_providers`
...
pub const CHATGPT_WSAI_CODE_BASE_URL: &str = "https://chatgpt.com/backend-api/codex";
...
const CHAT_WIRE_API_REMOVED_ERROR: &str = "`wire_api = \"chat\"` is no longer supported.\nHow to fix: set `wire_api = \"responses\"` in your provider config.\nMore info: https://github.com/wsai/wsai-code/discussions/7782";
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These WSAI_CODE_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_codex_oss_base_url = format!(
        "http://localhost:{codex_oss_port}/v1",
        codex_oss_port = std::env::var("WSAI_CODE_OSS_PORT")
            .ok()
    ...
    let codex_oss_base_url = std::env::var("WSAI_CODE_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
