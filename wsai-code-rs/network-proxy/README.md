...
`codex-network-proxy` is WSAI CODE's local network policy enforcement proxy. It runs:
...
`codex-network-proxy` reads from WSAI CODE's merged `config.toml` (via `codex-core` config loading).
...
# CA cert/key are managed internally under $WSAI_CODE_HOME/proxy/ (ca.pem + ca.key).
# When MITM is active, spawned commands receive CA bundle env vars pointing at
# immutable bundles under $WSAI_CODE_HOME/proxy/ so common HTTPS clients trust the managed CA.
...
When `codex-network-proxy` is embedded in managed WSAI CODE runtime, policy decisions emit structured
