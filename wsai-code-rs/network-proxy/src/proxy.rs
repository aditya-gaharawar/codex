...
pub const PROXY_ACTIVE_ENV_KEY: &str = "WSAI_CODE_NETWORK_PROXY_ACTIVE";
pub const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "WSAI_CODE_NETWORK_ALLOW_LOCAL_BINDING";
...
#[cfg(target_os = "macos")]
pub const WSAI_CODE_PROXY_GIT_SSH_COMMAND_MARKER: &str = "WSAI_CODE_PROXY_GIT_SSH_COMMAND=1 ";
#[cfg(target_os = "macos")]
const WSAI_CODE_PROXY_GIT_SSH_COMMAND_PREFIX: &str =
    "WSAI_CODE_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x ";
#[cfg(target_os = "macos")]
const WSAI_CODE_PROXY_GIT_SSH_COMMAND_SUFFIX: &str = " %h %p'";
...
fn codex_proxy_git_ssh_command(socks_addr: SocketAddr) -> String {
    format!("{WSAI_CODE_PROXY_GIT_SSH_COMMAND_PREFIX}{socks_addr}{WSAI_CODE_PROXY_GIT_SSH_COMMAND_SUFFIX}")
}

#[cfg(target_os = "macos")]
fn is_codex_proxy_git_ssh_command(command: &str) -> bool {
    command.starts_with(WSAI_CODE_PROXY_GIT_SSH_COMMAND_PREFIX)
        && command.ends_with(WSAI_CODE_PROXY_GIT_SSH_COMMAND_SUFFIX)
}
...
mod tests {
    ...
    fn apply_proxy_env_overrides_sets_common_tool_vars() {
        ...
        assert_eq!(
            ...
            Some(
                &"WSAI_CODE_PROXY_GIT_SSH_COMMAND=1 ssh -o ProxyCommand='nc -X 5 -x 127.0.0.1:8081 %h %p'"
                    .to_string()
            )
        );
