...
/// Base config deserialized from ~/.wsai-code/config.toml.
...
pub struct ConfigToml {
    ...
    /// Defaults to `$WSAI_CODE_SQLITE_HOME` when set. Otherwise uses `$WSAI_CODE_HOME`.
    ...
    /// Defaults to `$WSAI_CODE_HOME/log`.
    ...
    /// directories for `.wsai-code` folders. Defaults to [".git"] when unset.
    #[serde(default)]
