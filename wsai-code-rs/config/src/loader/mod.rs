...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/wsai-code/config.toml";
...
///
/// - system    `/etc/wsai-code/requirements.toml` (Unix) or
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/wsai-code/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Codex\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${WSAI_CODE_HOME}/config.toml`
/// - profile   `${WSAI_CODE_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.wsai-code/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.wsai-code/config.toml` (loaded but disabled when untrusted)
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/wsai-code/requirements.toml"))
}
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".wsai-code"))
    }
}
...
async fn load_project_layers(
    ...
    for dir in dirs {
        let dot_codex_abs = dir.join(".wsai-code");
...
/// from a file like `/etc/wsai-code/managed_config.toml` that has the same
