...
impl ConfigLayerEntry {
    ...
    // Get the `.wsai-code/` folder associated with this config layer, if any.
    ...
    /// Returns the `.wsai-code/` folder that should be used for hook declarations.
    ///
...
pub struct ConfigLayerStack {
    ...
    /// the base `$WSAI_CODE_HOME/config.toml` layer followed by the profile override
...
impl ConfigLayerStack {
    ...
    /// the base `$WSAI_CODE_HOME/config.toml` layer because the active layer is the
