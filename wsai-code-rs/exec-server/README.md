...
must support it.
It uses the standard WSAI CODE ChatGPT sign-in state; run `codex login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `WSAI_CODE_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; WSAI CODE then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `WSAI_CODE_API_KEY`;
WSAI CODE sends it as a bearer token on the registration request. For example:

```sh
WSAI_CODE_API_KEY="$OPENAI_API_KEY" \
codex exec-server \
  --remote ... \
...
remote registration should use; the CLI builds that provider from WSAI CODE auth
