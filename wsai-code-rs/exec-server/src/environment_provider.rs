...
use crate::environment::WSAI_CODE_EXEC_SERVER_URL_ENV_VAR;
...
/// Default provider backed by `WSAI_CODE_EXEC_SERVER_URL`.
#[derive(Clone, Debug)]
...
impl DefaultEnvironmentProvider {
    /// Builds a provider from an already-read raw `WSAI_CODE_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self { exec_server_url }
    }

    /// Builds a provider by reading `WSAI_CODE_EXEC_SERVER_URL`.
    pub fn from_env() -> Self {
        Self::new(std::env::var(WSAI_CODE_EXEC_SERVER_URL_ENV_VAR).ok())
    }
