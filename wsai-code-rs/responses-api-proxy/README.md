...
# Add this to ~/.wsai-code/config.toml:
...
A non-privileged user would then run WSAI CODE as follows, specifying the `model_provider` dynamically:
...
- Authentication is fixed to `Authorization: Bearer <key>` to match the WSAI CODE CLI expectations.
...
- We leverage [`codex_process_hardening`](https://github.com/wsai/wsai-code/blob/main/wsai-code-rs/process-hardening/README.md) so `codex-responses-api-proxy` is run with standard process-hardening techniques.
