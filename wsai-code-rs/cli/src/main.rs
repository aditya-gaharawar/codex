...
/// WSAI CODE — Your AI coding agent
///
...
#[clap(
    ...
    // `wsai-code-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `wsai-code` command name that users run.
    bin_name = "wsai-code",
    override_usage = "wsai-code [OPTIONS] [PROMPT]\n       wsai-code [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    /// Run WSAI CODE non-interactively.
    ...
    /// Manage external MCP servers for WSAI CODE.
    Mcp(McpCli),

    /// Manage WSAI CODE plugins.
    Plugin(PluginCli),

    /// Start WSAI CODE as an MCP server (stdio).
    ...
    /// Launch the WSAI CODE desktop app (opens the app installer if missing).
    #[cfg(any(target_os = "macos", target_os = "windows"))]
    ...
    /// Update WSAI CODE to the latest version.
    Update,

    /// Diagnose local WSAI CODE installation, config, auth, and runtime health.
    Doctor(DoctorCommand),

    /// Run commands within a WSAI CODE-provided sandbox.
    ...
    /// Apply the latest diff produced by WSAI CODE agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from WSAI CODE Cloud and apply changes locally.
...
struct ReviewCommand {
    /// Error out when config.toml contains fields that are not recognized by this version of WSAI CODE.
...
struct UnsupportedSandboxArgs {
    /// Layer $WSAI_CODE_HOME/<name>.config.toml on top of the base user config.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | wsai-code login --with-api-key`)"
    )]
    with_api_key: bool,

    #[arg(
        long = "with-access-token",
        help = "Read the access token from stdin (e.g. `printenv WSAI_CODE_ACCESS_TOKEN | wsai-code login --with-access-token`)"
    )]
...
struct ExecServerCommand {
    ...
    /// Use Agent Identity auth from WSAI_CODE_ACCESS_TOKEN for remote registration.
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for WSAI CODE tooling.
    #[clap(hide = true)]
...
fn run_update_command() -> anyhow::Result<()> {
    ...
    {
        anyhow::bail!(
            "`wsai-code update` is not available in debug builds. Install a release build of WSAI CODE to use this command."
        );
    }
    ...
    #[cfg(not(debug_assertions))]
    {
        let Some(action) = codex_tui::get_update_action() else {
            anyhow::bail!(
                "Could not detect the WSAI CODE installation method. Please update manually: https://developers.openai.com/codex/cli/"
            );
        };
        run_update_action(action)
    }
...
async fn cli_main(
    ...
    if let Ok(home) = std::env::var("HOME") {
        let legacy = std::path::Path::new(&home).join(".wsai-code");
        if legacy.exists() {
            eprintln!(
                "[WSAI CODE] Legacy ~/.codex config detected. Please move it to ~/.wsai-code/"
            );
        }
    }
    ...
    match subcommand {
        ...
        Some(Subcommand::Review(ReviewCommand {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["wsai-code", "exec"])?;
            exec_cli
                .shared
                .inherit_exec_root_options(&interactive.shared);
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.with_api_key && login_cli.with_access_token {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | wsai-code login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Sandbox(mut sandbox_cli)) => {
            ...
            #[cfg(not(any(target_os = "macos", target_os = "linux", target_os = "windows")))]
            {
                ...
                anyhow::bail!("`wsai-code sandbox` is not supported on this operating system");
            }
        }
...
fn profile_v2_for_subcommand<'a>(
    ...
    match subcommand {
        ...
        _ => anyhow::bail!(
            "--profile only applies to runtime commands and `wsai-code mcp`: `codex`, `wsai-code exec`, `wsai-code review`, `wsai-code resume`, `wsai-code archive`, `wsai-code delete`, `wsai-code unarchive`, `wsai-code fork`, `wsai-code mcp`, `wsai-code sandbox`, and `wsai-code debug prompt-input`."
        ),
    }
}
...
async fn load_exec_server_remote_auth_provider(
    ...
    if use_agent_identity_auth {
        let agent_identity_jwt = read_codex_access_token_from_env().ok_or_else(|| {
            anyhow::anyhow!("WSAI_CODE_ACCESS_TOKEN is required when --use-agent-identity-auth is set")
        })?;
    ...
    let auth = load_exec_server_remote_auth(
        config,
        "remote exec-server registration requires ChatGPT authentication or API key authentication; run `wsai-code login` or set WSAI_CODE_API_KEY",
    )
    .await?;
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `wsai-code {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `wsai-code {subcommand}`"
        );
    }
    Ok(())
}
...
/// `wsai-code --strict-config` continue to work for the TUI and for wrappers that
...
fn reject_strict_config_for_unsupported_subcommand(
    ...
    if strict_config {
        anyhow::bail!("`--strict-config` is not supported for `wsai-code {subcommand}`");
    }
    Ok(())
}
...
/// Build the final `TuiCli` for a `wsai-code resume` invocation.
...
/// Build the final `TuiCli` for a `wsai-code fork` invocation.
...
mod tests {
    ...
    fn profile_v2_is_rejected_for_config_management_subcommands() {
        assert!(profile_v2_for_args(&["wsai-code", "--profile", "work", "features", "list"]).is_err());
    }

    #[test]
    fn profile_v2_is_allowed_for_runtime_subcommands() {
        assert_eq!(
            profile_v2_for_args(&["wsai-code", "--profile", "work", "resume"])
                .expect("resume supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["wsai-code", "--profile", "work", "debug", "prompt-input"])
                .expect("debug prompt-input supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["wsai-code", "--profile", "work", "mcp", "list"])
                .expect("mcp supports profile-v2")
                .as_deref(),
            Some("work")
        );
        assert_eq!(
            profile_v2_for_args(&["wsai-code", "--profile", "work", "sandbox"])
    ...
    fn import_remains_an_interactive_prompt() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "import"]).expect("parse");
    ...
    fn profile_v2_rejects_non_plain_names_at_parse_time() {
        assert!(
            MultitoolCli::try_parse_from(["wsai-code", "--profile", "nested/work", "resume"]).is_err()
        );
    }
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "exec", "--json", "resume", "--last", "2+2"])
    ...
    fn debug_models_parses_bundled_flag() {
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "debug", "models", "--bundled"]).expect("parse");
    ...
    fn plugin_marketplace_help_uses_plugin_namespace() {
        let help = help_from_args(&["wsai-code", "plugin", "marketplace", "--help"]);
        assert!(
        ...
        for (subcommand, usage) in [
            ...
            let help = help_from_args(&["wsai-code", "plugin", "marketplace", subcommand, "--help"]);
    ...
    fn plugin_marketplace_add_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "plugin", "marketplace", "add", "owner/repo"])
                .expect("parse");
    ...
    fn plugin_marketplace_upgrade_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "plugin", "marketplace", "upgrade", "debug"])
                .expect("parse");
    ...
    fn plugin_list_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "plugin", "list", "--marketplace", "debug"])
                .expect("parse");
    ...
    fn update_parses_as_update_subcommand() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "update"]).expect("parse");
    ...
    fn sandbox_parses_permissions_profile_short_alias() {
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "sandbox", "-P", ":workspace", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_parses_config_profile() {
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "sandbox", "--profile", "work", "--", "echo"])
                .expect("parse");
    ...
    fn sandbox_rejects_explicit_profile_controls_without_profile() {
        let err = MultitoolCli::try_parse_from(["wsai-code", "sandbox", "-C", "/tmp"])
    ...
    fn plugin_marketplace_remove_parses_under_plugin() {
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "plugin", "marketplace", "remove", "debug"])
                .expect("parse");
    ...
    fn marketplace_no_longer_parses_at_top_level() {
        let add_result =
            MultitoolCli::try_parse_from(["wsai-code", "marketplace", "add", "owner/repo"]);
        assert!(add_result.is_err());

        let upgrade_result =
            MultitoolCli::try_parse_from(["wsai-code", "marketplace", "upgrade", "debug"]);
        assert!(upgrade_result.is_err());

        let remove_result =
            MultitoolCli::try_parse_from(["wsai-code", "marketplace", "remove", "debug"]);
        assert!(remove_result.is_err());
    }

    #[test]
    fn full_auto_no_longer_parses_at_top_level() {
        let result = MultitoolCli::try_parse_from(["wsai-code", "--full-auto"]);
    ...
    fn exec_full_auto_reports_migration_path() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "exec", "--full-auto", "summarize"])
    ...
    fn sandbox_full_auto_no_longer_parses() {
        let result = MultitoolCli::try_parse_from(["wsai-code", "sandbox", "--full-auto", "--"]);
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["wsai-code", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["wsai-code", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["wsai-code", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_last_accepts_prompt_positional() {
        let interactive = finalize_resume_from_args(
            ["wsai-code", "resume", "--last", "/compact focus on auth"].as_ref(),
        );
    ...
    fn resume_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["wsai-code", "resume", "--last", "1234", "continue here"])
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["wsai-code", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_resume_from_args(["wsai-code", "resume", "1234", "continue here"].as_ref());
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["wsai-code", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["wsai-code", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_bypass_hook_trust_flag() {
        let interactive = finalize_resume_from_args(
            ["wsai-code", "resume", "--dangerously-bypass-hook-trust"].as_ref(),
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["wsai-code", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["wsai-code", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_last_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["wsai-code", "fork", "--last", "/compact focus on auth"].as_ref());
    ...
    fn fork_last_rejects_explicit_session_and_prompt() {
        let err =
            MultitoolCli::try_parse_from(["wsai-code", "fork", "--last", "1234", "continue here"])
                .expect_err("--last with an explicit session and prompt should be rejected");
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["wsai-code", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_with_session_id_accepts_prompt_positional() {
        let interactive =
            finalize_fork_from_args(["wsai-code", "fork", "1234", "continue here"].as_ref());
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["wsai-code", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["wsai-code", "app-server"].as_ref());
    ...
    fn app_server_remote_control_startup_flag_enables_remote_control() {
        let enabled = app_server_from_args(["wsai-code", "app-server", "--remote-control"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["wsai-code", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn strict_config_parses_for_supported_commands() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "--strict-config"]).expect("parse");
        assert!(cli.interactive.strict_config);

        let cli = MultitoolCli::try_parse_from(["wsai-code", "mcp-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
        ...
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "review", "--strict-config", "--uncommitted"])
                .expect("parse");
        assert_matches!(
            cli.subcommand,
            Some(Subcommand::Review(ReviewCommand {
        ...
        let cli = MultitoolCli::try_parse_from(["wsai-code", "exec-server", "--strict-config"])
            .expect("parse");
        assert_matches!(
            cli.subcommand,
    ...
    fn root_strict_config_is_supported_for_exec_server() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "--strict-config", "exec-server"])
            .expect("parse");
    ...
    fn root_strict_config_is_rejected_for_unsupported_subcommands() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "--strict-config", "mcp", "list"])
            .expect("parse");
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `wsai-code mcp`"
        );

        let cli = MultitoolCli::try_parse_from(["wsai-code", "--strict-config", "remote-control"])
            .expect("parse");
        let err = reject_root_strict_config_for_subcommand(
            cli.interactive.strict_config,
            &cli.subcommand,
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `wsai-code remote-control`"
        );
    }
    ...
    fn app_server_subcommands_reject_strict_config() {
        let app_server =
            app_server_from_args(["wsai-code", "app-server", "--strict-config", "proxy"].as_ref());
        ...
        assert_eq!(
            err.to_string(),
            "`--strict-config` is not supported for `wsai-code app-server proxy`"
        );
    }

    #[test]
    fn reject_remote_flag_for_remote_control() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "--remote", "unix://", "remote-control"])
            .expect("parse");
    ...
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "--remote", "unix://codex.sock"])
            .expect("parse");
    ...
    fn remote_auth_token_env_flag_parses_for_interactive_root() {
        ...
        assert_eq!(
            cli.remote.remote_auth_token_env.as_deref(),
            Some("WSAI_CODE_REMOTE_AUTH_TOKEN")
        );
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["wsai-code", "resume", "--remote", "unix://codex.sock"])
                .expect("parse");
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("WSAI_CODE_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("WSAI_CODE_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["wsai-code", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["wsai-code", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            codex_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_stdio_flag_parses() {
        let app_server = app_server_from_args(["wsai-code", "app-server", "--stdio"].as_ref());
    ...
    fn app_server_listen_unix_socket_url_parses() {
        let app_server =
            app_server_from_args(["wsai-code", "app-server", "--listen", "unix://"].as_ref());
        assert_eq!(
            app_server.listen,
    ...
    fn app_server_listen_unix_socket_path_parses() {
        let app_server = app_server_from_args(
            ["wsai-code", "app-server", "--listen", "unix:///tmp/codex.sock"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            codex_app_server::AppServerTransport::UnixSocket {
    ...
    fn app_server_listen_off_parses() {
        let app_server = app_server_from_args(["wsai-code", "app-server", "--listen", "off"].as_ref());
    ...
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["wsai-code", "app-server", "--listen", "http://foo"]);
        assert!(parse_result.is_err());
    }

    #[test]
    fn app_server_proxy_subcommand_parses() {
        let app_server = app_server_from_args(["wsai-code", "app-server", "proxy"].as_ref());
        assert!(matches!(
    ...
    fn app_server_daemon_subcommands_parse() {
        ...
        assert!(matches!(
            app_server_from_args(["wsai-code", "app-server", "daemon", "start"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Start
            }))
        ));
        assert!(matches!(
            app_server_from_args(["wsai-code", "app-server", "daemon", "restart"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["wsai-code", "app-server", "daemon", "enable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(
                ["wsai-code", "app-server", "daemon", "disable-remote-control"].as_ref()
            )
            .subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
        ...
        assert!(matches!(
            app_server_from_args(["wsai-code", "app-server", "daemon", "stop"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
                subcommand: AppServerDaemonSubcommand::Stop
            }))
        ));
        assert!(matches!(
            app_server_from_args(["wsai-code", "app-server", "daemon", "version"].as_ref()).subcommand,
            Some(AppServerSubcommand::Daemon(AppServerDaemonCommand {
    ...
    fn app_server_proxy_sock_path_parses() {
        let app_server =
            app_server_from_args(["wsai-code", "app-server", "proxy", "--sock", "codex.sock"].as_ref());
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["wsai-code", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
    ...
    fn strict_config_feature_toggle_error(args: &[&str]) -> anyhow::Error {
        let cli_args = std::iter::once("wsai-code")
