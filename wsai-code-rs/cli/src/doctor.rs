...
use codex_login::WSAI_CODE_ACCESS_TOKEN_ENV_VAR;
use codex_login::WSAI_CODE_API_KEY_ENV_VAR;
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("WSAI_CODE_MANAGED_BY_BUN").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "WSAI_CODE_MANAGED_PACKAGE_ROOT",
    );
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g @wsai/code would update a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update Codex so the JS shim provides WSAI_CODE_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("WSAI_CODE_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("WSAI_CODE_MANAGED_BY_NPM").is_none()
        && env::var_os("WSAI_CODE_MANAGED_BY_BUN").is_none()
    {
        return false;
    }
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) = env::var_os("WSAI_CODE_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@wsai").join("code");
...
fn codex_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["wsai-code"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "wsai-code"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("WSAI_CODE_HOME: {}", config.codex_home.display()));
...
fn auth_check(config: &Config) -> DoctorCheck {
    ...
    let env_auth_vars = [
        ...
        WSAI_CODE_API_KEY_ENV_VAR,
        WSAI_CODE_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        codex_app_server_protocol::AuthMode::ApiKey => {
            ...
            let env_key_present =
                env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(WSAI_CODE_API_KEY_ENV_VAR);
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["WSAI_CODE_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "WSAI_CODE_HOME", &config.codex_home);
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match codex_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "WSAI_CODE_HOME was resolved without config",
        )
        .detail(format!("WSAI_CODE_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "WSAI_CODE_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(WSAI_CODE_API_KEY_ENV_VAR) {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(WSAI_CODE_ACCESS_TOKEN_ENV_VAR) {
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@wsai/code");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@wsai").join("code")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@wsai/code");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@wsai").join("code"),
            }
        );
    }
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "WSAI_CODE_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("WSAI_CODE_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
