...
const WSAI_CODE_WINDOWS_INSTALLER_URL: &str =
    "https://get.microsoft.com/installer/download/9PLM9XGG6VKS?cid=website_cta_psi";
const WSAI_CODE_MICROSOFT_STORE_WEB_URL: &str = "https://apps.microsoft.com/detail/9plm9xgg6vks";
...
pub async fn run_windows_app_open_or_install(
    ...
    let download_url = download_url_override
        .as_deref()
        .unwrap_or(WSAI_CODE_WINDOWS_INSTALLER_URL);
    if open_url(download_url).await.is_err() && download_url_override.is_none() {
        open_url(WSAI_CODE_MICROSOFT_STORE_WEB_URL).await?;
    }
