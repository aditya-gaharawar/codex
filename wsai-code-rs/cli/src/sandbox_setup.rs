...
pub(crate) struct SandboxSetupCommand {
    ...
    /// WSAI_CODE_HOME for the Codex user. Required with --user.
...
mod tests {
    ...
    fn parses_managed_user_identity() {
        ...
        assert_eq!(
            ...
            Some(std::path::Path::new(r"C:\Users\alice\.wsai-code"))
        );
    }
    ...
    fn parses_setup_from_sandbox_command_args() {
        let command = parse_setup_command(&[
            ...
            r"C:\Users\alice\.wsai-code".to_string(),
        ])
        .expect("parse")
