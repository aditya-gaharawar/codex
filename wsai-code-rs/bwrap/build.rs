...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=WSAI_CODE_BWRAP_SOURCE_DIR");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_ALLOW_CROSS");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_PATH");
    println!("cargo:rerun-if-env-changed=PKG_CONFIG_SYSROOT_DIR");
    println!("cargo:rerun-if-env-changed=WSAI_CODE_SKIP_BWRAP_BUILD");
    ...
    if target_os != "linux" || env::var_os("WSAI_CODE_SKIP_BWRAP_BUILD").is_some() {
        return;
    }
...
///
/// Priority:
/// 1. `WSAI_CODE_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
/// 2. The vendored bubblewrap tree under `wsai-code-rs/vendor/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("WSAI_CODE_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "WSAI_CODE_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set WSAI_CODE_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under wsai-code-rs/vendor.",
        vendor_dir.display()
    ))
}
