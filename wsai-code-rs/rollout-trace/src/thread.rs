...
pub const WSAI_CODE_ROLLOUT_TRACE_ROOT_ENV: &str = "WSAI_CODE_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `WSAI_CODE_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(WSAI_CODE_ROLLOUT_TRACE_ROOT_ENV) else {
