...
> **Privacy:** Rollout tracing is not telemetry. WSAI CODE does **not** upload or
> report these traces; it writes local bundles only when
> `WSAI_CODE_ROLLOUT_TRACE_ROOT` is set. Those local bundles can contain prompts,
...
during a WSAI CODE session. It records raw runtime evidence into a local bundle on
...
Hot-path WSAI CODE code does not try to build the final graph while the session is
...
model-visible conversation plus the runtime objects that explain how WSAI CODE got
there.
...
one from `WSAI_CODE_ROLLOUT_TRACE_ROOT`; fresh spawned child threads derive their
...
WSAI CODE session fail just because diagnostic recording failed. Core emits raw
...
WSAI CODE rollout/session being observed. Keeping those separate lets us reason about
