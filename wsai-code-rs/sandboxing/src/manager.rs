...
use crate::landlock::WSAI_CODE_LINUX_SANDBOX_ARG0;
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(WSAI_CODE_LINUX_SANDBOX_ARG0) {
        os_string_to_command_component(exe.as_os_str().to_owned())
    } else {
        WSAI_CODE_LINUX_SANDBOX_ARG0.to_string()
    }
}
