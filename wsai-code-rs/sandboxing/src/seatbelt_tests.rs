...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.wsai-code".to_string(),
...
fn create_seatbelt_args_with_read_only_git_and_codex_subpaths() {
    ...
    // write to .wsai-code/config.toml in the vulnerable root.
    ...
    // Verify that .wsai-code/config.toml cannot be modified under the generated
    // Seatbelt policy.
...
fn create_seatbelt_args_block_first_time_dot_codex_creation_with_metadata_name_regex() {
    ...
    let dot_codex = repo_root.join(".wsai-code");
...
struct PopulatedTmp {
    ...
    /// ran `git commit` themselves, or modified .wsai-code/config.toml to
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".wsai-code")).expect("create .codex");
    fs::write(
        vulnerable_root.join(".wsai-code").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .wsai-code/config.toml");
    ...
    let dot_codex_canonical = vulnerable_root_canonical.join(".wsai-code");
