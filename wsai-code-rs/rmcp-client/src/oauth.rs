...
//!
//! If the keyring is not available or fails, we fall back to WSAI_CODE_HOME/.credentials.json which is consistent with other coding CLI agents.
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create WSAI_CODE_HOME temp dir");
            unsafe {
                std::env::set_var("WSAI_CODE_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempCodexHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("WSAI_CODE_HOME");
            }
        }
    }
