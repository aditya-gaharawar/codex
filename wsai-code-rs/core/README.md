...
This crate implements the business logic for WSAI CODE. It is designed to be used by the various WSAI CODE UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //wsai-code-rs/core:core-all-wine-exec-test`. Temporary blockers use a
...
binary shipped with WSAI CODE and WSAI CODE surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
WSAI CODE also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so WSAI CODE rejects sandboxed shell commands that would enter the
