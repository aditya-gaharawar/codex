...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = codex_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("wsai-code-rs"));
...
/// avoids clobbering a developer’s real `~/.wsai-code`.
...
pub fn sandbox_env_var() -> &'static str {
    codex_core::spawn::WSAI_CODE_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    codex_core::spawn::WSAI_CODE_SANDBOX_NETWORK_DISABLED_ENV_VAR
}
