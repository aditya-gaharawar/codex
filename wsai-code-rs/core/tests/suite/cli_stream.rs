...
fn personal_access_token_exec_command(server: &MockServer, home: &TempDir) -> Command {
    ...
    cmd.arg("exec")
        ...
        .arg(repo_root())
        .arg("hello?");
    cmd.env("WSAI_CODE_HOME", home.path())
        .env(WSAI_CODE_ACCESS_TOKEN_ENV_VAR, PERSONAL_ACCESS_TOKEN)
        .env("WSAI_CODE_AUTHAPI_BASE_URL", server.uri())
        .env_remove(WSAI_CODE_API_KEY_ENV_VAR)
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    cmd.arg("exec")
        ...
        .arg(&prompt);
    cmd.env("WSAI_CODE_HOME", home.path())
        .env(WSAI_CODE_API_KEY_ENV_VAR, "dummy");
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("WSAI_CODE_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy");

    let output2 = run_cli_command(&mut cmd2).unwrap();
    assert!(output2.status.success(), "resume wsai-code-cli run failed");
