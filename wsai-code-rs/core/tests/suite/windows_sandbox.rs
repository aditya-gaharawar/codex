...
fn codex_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestCodexHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        ...
        // retries run in the same Windows VM, so keep WSAI_CODE_HOME stable within
        ...
        std::fs::create_dir_all(&codex_home)
            .with_context(|| format!("create stable test WSAI_CODE_HOME {}", codex_home.display()))?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    ...
    let _codex_home_guard = EnvVarGuard::set("WSAI_CODE_HOME", codex_home.path().as_os_str());
