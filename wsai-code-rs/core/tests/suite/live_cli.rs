...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let codex_home = home.path().join(".wsai-code");
    ...
    let mut cmd = Command::new(codex_utils_cargo_bin::cargo_bin("wsai-code-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn wsai-code-rs");
