...
fn write_sample_plugin_mcp(codex_home: &std::path::Path) {
    ...
    std::fs::create_dir_all(plugin_root.join(".wsai-code-plugin")).expect("create plugin manifest dir");
    std::fs::write(
...
fn openai_file_params_are_only_honored_for_codex_apps() {
    ...
    assert_eq!(
        openai_file_input_params_for_server(WSAI_CODE_APPS_MCP_SERVER_NAME, meta),
...
fn codex_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: WSAI_CODE_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_WSAI_CODE_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_WSAI_CODE_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
fn mcp_tool_call_item_metadata_only_trusts_codex_apps_identity() {
    ...
    assert_eq!(
        McpToolCallItemMetadata::from_tool_metadata(WSAI_CODE_APPS_MCP_SERVER_NAME, Some(&metadata),),
...
async fn codex_apps_tool_call_request_meta_includes_turn_metadata_and_codex_apps_meta() {
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_WSAI_CODE_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_WSAI_CODE_APPS_META_KEY: {
...
async fn codex_apps_auth_elicitation_feature_enabled_requests_elicitation() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(request.server_name, WSAI_CODE_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_codex_dir = project_dir.path().join(".wsai-code");
