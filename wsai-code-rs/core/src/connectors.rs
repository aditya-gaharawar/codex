...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    mcp_servers.retain(|name, _| name == WSAI_CODE_APPS_MCP_SERVER_NAME);
    ...
    let refreshed_tools = if force_refetch {
            ...
            .await
        {
            ...
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{WSAI_CODE_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let codex_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(WSAI_CODE_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(WSAI_CODE_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(WSAI_CODE_APPS_MCP_SERVER_NAME, timeout)
                .await;
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        if tool.server_name != WSAI_CODE_APPS_MCP_SERVER_NAME {
            return None;
        }
...
pub(crate) fn mcp_approvals_reviewer(
    ...
    let app_reviewer = if server_name == WSAI_CODE_APPS_MCP_SERVER_NAME {
