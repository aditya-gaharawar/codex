...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.wsai-code/config.toml` snippet:
    ///
    /// ```toml
    ...
    ///          https://github.com/wsai/wsai-code/blob/main/wsai-code-rs/rmcp-client/src/oauth.rs#L2
    /// file: WSAI_CODE_HOME/.credentials.json
    ...
    /// Directory containing all Codex state (defaults to `~/.wsai-code` but can be
    /// overridden by the `WSAI_CODE_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Directory where Codex stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where Codex writes log files (defaults to `$WSAI_CODE_HOME/log`).
    pub log_dir: PathBuf,
    ...
    /// Settings that govern if and what will be written to `~/.wsai-code/history.jsonl`.
    pub history: History,
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .wsai-code/ folders.
...
/// Patch `WSAI_CODE_HOME/config.toml` project state to set trust level.
/// Use with caution.
