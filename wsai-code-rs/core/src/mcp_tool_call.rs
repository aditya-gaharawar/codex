...
use codex_mcp::WSAI_CODE_APPS_MCP_SERVER_NAME;
use codex_mcp::MCP_TOOL_WSAI_CODE_APPS_META_KEY;
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == WSAI_CODE_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == WSAI_CODE_APPS_MCP_SERVER_NAME {
    ...
    if server == WSAI_CODE_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
...
impl McpToolCallItemMetadata {
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        let trusted_mcp_app_metadata = if server == WSAI_CODE_APPS_MCP_SERVER_NAME {
            metadata
        } else {
            None
        };
...
async fn maybe_request_codex_apps_auth_elicitation(
    ...
    let params = McpServerElicitationRequestParams {
        ...
        server_name: WSAI_CODE_APPS_MCP_SERVER_NAME.to_string(),
...
async fn maybe_track_codex_app_used(
    ...
    if server != WSAI_CODE_APPS_MCP_SERVER_NAME {
        return;
    }
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_WSAI_CODE_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }
    ...
    if server == WSAI_CODE_APPS_MCP_SERVER_NAME {
        ...
        request_meta.insert(
            MCP_TOOL_WSAI_CODE_APPS_META_KEY.to_string(),
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == WSAI_CODE_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == WSAI_CODE_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        codex_apps_meta: tool_info
            ...
            .meta
            .as_ref()
            .and_then(|meta| meta.get(MCP_TOOL_WSAI_CODE_APPS_META_KEY))
...
fn openai_file_input_params_for_server(
    ...
    (server == WSAI_CODE_APPS_MCP_SERVER_NAME)
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == WSAI_CODE_APPS_MCP_SERVER_NAME {
