...
#[cfg(target_os = "macos")]
use crate::spawn::WSAI_CODE_SANDBOX_ENV_VAR;
use crate::spawn::WSAI_CODE_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
impl ExecRequest {
    ...
    pub(crate) fn from_sandbox_exec_request(
        ...
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                WSAI_CODE_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
        if sandbox == SandboxType::MacosSeatbelt {
            env.insert(WSAI_CODE_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        }
        Self {
            command,
            cwd,
