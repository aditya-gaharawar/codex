...
fn codex_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{WSAI_CODE_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| WSAI_CODE_APPS_MCP_SERVER_NAME.to_string());
...
"#
        ...
        assert_eq!(
            mcp_approvals_reviewer(&config, WSAI_CODE_APPS_MCP_SERVER_NAME, Some("calendar")),
            expected_app
        );
        assert_eq!(
            mcp_approvals_reviewer(&config, WSAI_CODE_APPS_MCP_SERVER_NAME, Some("drive")),
            expected_default
        );
        assert_eq!(
