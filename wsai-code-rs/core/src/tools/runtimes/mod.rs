...
*/
use crate::exec_env::WSAI_CODE_THREAD_ID_ENV_VAR;
...
#[cfg(target_os = "macos")]
use codex_network_proxy::WSAI_CODE_PROXY_GIT_SSH_COMMAND_MARKER;
...
pub(crate) fn strip_managed_proxy_env(env: &mut HashMap<String, String>) {
    ...
    if env
        ...
        .is_some_and(|command| command.starts_with(WSAI_CODE_PROXY_GIT_SSH_COMMAND_MARKER))
    {
...
/// runtime-only vars like `WSAI_CODE_THREAD_ID` without pretending they came from
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    if let Some(thread_id) = env.get(WSAI_CODE_THREAD_ID_ENV_VAR) {
        override_env.insert(WSAI_CODE_THREAD_ID_ENV_VAR.to_string(), thread_id.clone());
    }
...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    build_override_exports_for_keys("__WSAI_CODE_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__WSAI_CODE_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__WSAI_CODE_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__WSAI_CODE_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
...
fn build_codex_proxy_git_ssh_command_exports() -> (String, String) {
    ...
    let marker_pattern = format!("{}\\ *", WSAI_CODE_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__WSAI_CODE_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
