...
use crate::client::X_WSAI_CODE_INSTALLATION_ID_HEADER;
use crate::client::X_WSAI_CODE_PARENT_THREAD_ID_HEADER;
use crate::client::X_WSAI_CODE_TURN_METADATA_HEADER;
use crate::client::X_WSAI_CODE_WINDOW_ID_HEADER;
...
const RESERVED_METADATA_KEYS: &[&str] = &[
    INSTALLATION_ID_KEY,
    X_WSAI_CODE_INSTALLATION_ID_HEADER,
    SESSION_ID_KEY,
    THREAD_ID_KEY,
    TURN_ID_KEY,
    WINDOW_ID_KEY,
    X_WSAI_CODE_WINDOW_ID_HEADER,
    X_WSAI_CODE_TURN_METADATA_HEADER,
    X_WSAI_CODE_PARENT_THREAD_ID_HEADER,
...
impl CodexResponsesMetadata {
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        let mut client_metadata = HashMap::from([
            (
                X_WSAI_CODE_INSTALLATION_ID_HEADER.to_string(),
                self.installation_id.clone(),
            ),
            (SESSION_ID_KEY.to_string(), self.session_id.clone()),
            (THREAD_ID_KEY.to_string(), self.thread_id.clone()),
            (X_WSAI_CODE_WINDOW_ID_HEADER.to_string(), self.window_id.clone()),
        ]);
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_WSAI_CODE_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(X_WSAI_CODE_TURN_METADATA_HEADER.to_string(), turn_metadata_json);
        }
        client_metadata
    }
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_WSAI_CODE_WINDOW_ID_HEADER, &self.window_id);
