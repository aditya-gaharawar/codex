...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "WSAI_CODE_NETWORK_ALLOW_LOCAL_BINDING";
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, codex_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to WSAI_CODE_HOME or anything under WSAI_CODE_HOME/.sandbox,
    // WSAI_CODE_HOME/.sandbox-bin, or WSAI_CODE_HOME/.sandbox-secrets. These locations contain sandbox
...
mod tests {
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "WSAI_CODE_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_codex = extra_write_root.join(".wsai-code");
