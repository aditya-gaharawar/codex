...
pub const WSAI_CODE_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const WSAI_CODE_HOME_FLAG: &str = "--codex-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        WSAI_CODE_WINDOWS_SANDBOX_ARG1.to_string(),
        WSAI_CODE_HOME_FLAG.to_string(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            WSAI_CODE_HOME_FLAG => codex_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?)),
            COMMAND_CWD_FLAG => {
    ...
    let codex_home = codex_home.ok_or_else(|| anyhow!("missing required {WSAI_CODE_HOME_FLAG}"))?;
    if !codex_home.is_absolute() {
        bail!(
            "{WSAI_CODE_HOME_FLAG} must be absolute: {}",
            codex_home.display()
        );
    }
