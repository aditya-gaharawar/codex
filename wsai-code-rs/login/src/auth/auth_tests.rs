...
async fn login_with_access_token_writes_only_personal_access_token() {
    ...
    Mock::given(method("GET"))
        ...
        .mount(&server)
        .await;
    let _authapi_guard = EnvVarGuard::set("WSAI_CODE_AUTHAPI_BASE_URL", &server.uri());
...
async fn chatgpt_auth_registers_agent_identity_when_enabled() -> anyhow::Result<()> {
    ...
    Mock::given(method("POST"))
        ...
        .and(body_partial_json(json!({
            "abom": {
                "agent_harness_id": "wsai-code-cli",
            },
...
fn remove_access_token_env_var() -> EnvVarGuard {
    EnvVarGuard::remove(WSAI_CODE_ACCESS_TOKEN_ENV_VAR)
}
...
async fn load_auth_reads_access_token_from_env() {
    ...
    let _access_token_guard = EnvVarGuard::set(WSAI_CODE_ACCESS_TOKEN_ENV_VAR, &agent_identity);
...
async fn load_auth_reads_personal_access_token_from_env() {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    let _authapi_guard = EnvVarGuard::set("WSAI_CODE_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard = EnvVarGuard::set(WSAI_CODE_ACCESS_TOKEN_ENV_VAR, "at-env-test");
...
async fn auth_manager_rejects_env_personal_access_token_workspace_mismatch() {
    ...
    let _access_token_guard =
        EnvVarGuard::set(WSAI_CODE_ACCESS_TOKEN_ENV_VAR, "at-env-workspace-mismatch");
...
async fn personal_access_token_does_not_offer_unauthorized_recovery() {
    ...
    let _authapi_guard = EnvVarGuard::set("WSAI_CODE_AUTHAPI_BASE_URL", &server.uri());
    let _access_token_guard =
        EnvVarGuard::set(WSAI_CODE_ACCESS_TOKEN_ENV_VAR, "at-no-unauthorized-recovery");
    let manager = Arc::new(
        AuthManager::new(
            codex_home.path().to_path_buf(),
            /*enable_codex_api_key_env*/ false,
...
async fn load_auth_keeps_codex_api_key_env_precedence() {
    ...
    let agent_identity = fake_agent_identity_jwt(&record).expect("fake agent identity");
    let _access_token_guard = EnvVarGuard::set(WSAI_CODE_ACCESS_TOKEN_ENV_VAR, &agent_identity);
    let _api_key_guard = EnvVarGuard::set(WSAI_CODE_API_KEY_ENV_VAR, "sk-env");
...
async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
    let _guard = EnvVarGuard::set(WSAI_CODE_API_KEY_ENV_VAR, "sk-env");
    let _access_token_guard = remove_access_token_env_var();
    let codex_home = tempdir().unwrap();
