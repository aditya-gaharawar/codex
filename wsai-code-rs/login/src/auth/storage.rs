...
/// Expected structure for $WSAI_CODE_HOME/auth.json.
...
impl FileAuthStorage {
    ...
    /// Attempt to read and parse the `auth.json` file in the given `WSAI_CODE_HOME` directory.
...
static WSAI_CODE_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("WSAI_CODE_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("WSAI_CODE_AUTH should be a valid secret name: {err}"),
    });
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &WSAI_CODE_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        self.secrets_manager
            .set(&SecretScope::Global, &WSAI_CODE_AUTH_SECRET_NAME, &serialized)
            .map_err(|err| {
                let message =
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &WSAI_CODE_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
