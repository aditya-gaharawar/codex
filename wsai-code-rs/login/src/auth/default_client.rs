...
/// See https://github.com/wsai/wsai-code/pull/3388/files for an example of what that would look like.
...
pub const WSAI_CODE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "WSAI_CODE_INTERNAL_ORIGINATOR_OVERRIDE";
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(WSAI_CODE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(WSAI_CODE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
/// policy, then layers in shared custom CA handling from `WSAI_CODE_CA_CERTIFICATE` /
...
fn is_sandboxed() -> bool {
    std::env::var("WSAI_CODE_SANDBOX").as_deref() == Ok("seatbelt")
}
