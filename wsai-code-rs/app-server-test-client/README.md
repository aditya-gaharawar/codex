...
Run from `<reporoot>/wsai-code-rs`.

```bash
# 1) Build debug codex binary
cargo build -p wsai-code-cli --bin codex
...
When WSAI CODE asks a question, choose a numbered option (or `o` for a free-form answer when offered)
...
active WSAI CODE profile must be authenticated. On a fresh local cache, the command
retries ephemeral turns while the installed remote bundle finishes syncing.

```bash
# Build a debug WSAI CODE binary; analytics capture is unavailable in release builds.
cargo build -p wsai-code-cli --bin codex
...
Cleanup does not require analytics capture or a debug WSAI CODE binary. When the
