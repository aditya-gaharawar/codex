...
fn snapshot_collects_metrics_without_shutdown() -> Result<()> {
    ...
    .with_tag("service", "wsai-code-cli")?
    .with_runtime_reader();
    ...
    let expected = BTreeMap::from([
        ("service".to_string(), "wsai-code-cli".to_string()),
...
fn manager_snapshot_metrics_collects_without_shutdown() -> Result<()> {
    let exporter = InMemoryMetricExporter::default();
    let config = MetricsConfig::in_memory("test", "wsai-code-cli", env!("CARGO_PKG_VERSION"), exporter)
        .with_tag("service", "wsai-code-cli")?
        .with_runtime_reader();
    let metrics = MetricsClient::new(config)?;
