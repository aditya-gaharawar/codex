# Bazel in wsai-code-rs

This repository uses Bazel to build the Rust workspace under `wsai-code-rs`.
...
- `rules_rs` imports third-party crates from `wsai-code-rs/Cargo.toml` and
  `wsai-code-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
...
- Each crate in `wsai-code-rs/*/BUILD.bazel` typically uses `codex_rust_crate` and
...
WSAI CODE uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `wsai/wsai-code` | Yes | Yes |
| `workflow_dispatch` in `wsai/wsai-code` | Yes | Yes |
| Same-repository pull request in `wsai/wsai-code` | Yes | Yes |
| Fork pull request into `wsai/wsai-code` | No | No; local |
...
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/codex \
  ...
  build --config=ci-linux //wsai-code-rs/cli:codex
```

The wrapper selects the OpenAI host only inside GitHub Actions for a trusted
run in `wsai/wsai-code`. A missing or malformed pull request event
