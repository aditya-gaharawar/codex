# WSAI CODE MCP Server Interface [experimental]

This document describes WSAI CODE's experimental MCP server interface: a JSON-RPC API that runs over the Model Context Protocol (MCP) transport to control a local WSAI CODE engine.
...
WSAI CODE exposes MCP-compatible methods to manage threads, turns, accounts, config, and approvals. The types live in `app-server-protocol/src/protocol/{common,v1,v2}.rs` and are consumed by the app server implementation in `app-server/`.
...
Run WSAI CODE as an MCP server and connect an MCP client:
...
Fetch the catalog of models available in the current WSAI CODE build with `model/list`. The request accepts optional pagination inputs:
...
- `codex/event` with the serialized WSAI CODE event payload. The shape matches `core/src/protocol.rs`'s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `codex` and `codex-reply` tools return standard MCP `CallToolResult` payloads. For compatibility with MCP clients that prefer `structuredContent`, WSAI CODE mirrors the content blocks inside `structuredContent` alongside the `threadId`.

Example:

```json
{
  "content": [{ "type": "text", "text": "Hello from WSAI CODE" }],
  "structuredContent": {
    "threadId": "019bbed6-1e9e-7f31-984c-a05b65045719",
    "content": "Hello from WSAI CODE"
  }
}
```
...
When WSAI CODE needs approval to apply changes or run commands, the server issues JSON-RPC requests to the client:
