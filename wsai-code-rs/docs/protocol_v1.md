...
These are entities exit on the codex backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `WSAI CODE` core system.
...
2. `Session`
   - The `WSAI CODE`'s current configuration and state
   - `WSAI CODE` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `WSAI CODE` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `WSAI CODE` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `WSAI CODE`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `WSAI CODE`, as `WSAI CODE` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `WSAI CODE` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `WSAI CODE`)
...
- `Event`
  - These are messages sent on the `EQ` (`WSAI CODE` -> UI)
...
sequenceDiagram
    ...
    box Daemon
    participant codex as WSAI CODE
