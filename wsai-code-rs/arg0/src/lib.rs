...
use codex_apply_patch::WSAI_CODE_CORE_APPLY_PATCH_ARG1;
use codex_exec_server::WSAI_CODE_FS_HELPER_ARG1;
use codex_install_context::InstallContext;
use codex_sandboxing::landlock::WSAI_CODE_LINUX_SANDBOX_ARG0;
use codex_utils_home_dir::find_codex_home;
#[cfg(target_os = "windows")]
use codex_windows_sandbox::WSAI_CODE_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == WSAI_CODE_LINUX_SANDBOX_ARG0 {
    ...
    if argv1 == WSAI_CODE_FS_HELPER_ARG1 {
        codex_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == WSAI_CODE_WINDOWS_SANDBOX_ARG1 {
        codex_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == WSAI_CODE_CORE_APPLY_PATCH_ARG1 {
        ...
        let exit_code = match patch_arg {
            ...
            None => {
                eprintln!("Error: {WSAI_CODE_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
...
///
/// 1.  Load `.env` values from `~/.wsai-code/.env` before creating any threads.
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "WSAI_CODE_";

/// Load env vars from ~/.wsai-code/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `WSAI_CODE_`.
fn load_dotenv() {
...
/// Helper to set vars from a dotenvy iterator while filtering out `WSAI_CODE_` keys.
...
fn prepare_path_entry_for_codex_aliases(
    ...
    // Use a WSAI_CODE_HOME-scoped temp root to avoid cluttering the top-level directory.
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {WSAI_CODE_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        ...
        codex_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(WSAI_CODE_LINUX_SANDBOX_ARG0))
            }
