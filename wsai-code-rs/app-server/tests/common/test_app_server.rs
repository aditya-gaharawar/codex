...
use codex_login::default_client::WSAI_CODE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "WSAI_CODE_APP_SERVER_DISABLE_MANAGED_CONFIG";
...
impl TestAppServer {
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.env("WSAI_CODE_HOME", codex_home);
        cmd.env("RUST_LOG", "warn");
        // Keep integration tests isolated from host managed configuration.
        cmd.env(
            "WSAI_CODE_APP_SERVER_MANAGED_CONFIG_PATH",
            codex_home.join("managed_config.toml"),
        );
        cmd.env_remove(WSAI_CODE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
