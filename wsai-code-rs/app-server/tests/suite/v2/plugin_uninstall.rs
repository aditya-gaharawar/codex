...
async fn plugin_uninstall_writes_remote_plugin_to_cloud_when_remote_plugin_enabled() -> Result<()> {
    ...
    std::fs::create_dir_all(remote_plugin_cache_root.join("1.0.0/.wsai-code-plugin"))?;
    std::fs::write(
        remote_plugin_cache_root.join("1.0.0/.wsai-code-plugin/plugin.json"),
    ...
    std::fs::create_dir_all(legacy_remote_plugin_cache_root.join("local/.wsai-code-plugin"))?;
...
async fn plugin_uninstall_uses_detail_scope_for_cache_namespace() -> Result<()> {
    ...
    std::fs::create_dir_all(workspace_cache_root.join("1.0.0/.wsai-code-plugin"))?;
    std::fs::write(
        workspace_cache_root.join("1.0.0/.wsai-code-plugin/plugin.json"),
        r#"{"name":"linear","version":"1.0.0"}"#,
    )?;
    let global_cache_root = codex_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear");
    std::fs::create_dir_all(global_cache_root.join("1.0.0/.wsai-code-plugin"))?;
