...
"#,
    ...
    std::fs::create_dir_all(workspace.path().join(".wsai-code"))?;
    std::fs::write(
        workspace.path().join(".wsai-code/config.toml"),
        r#"[features]
hooks = true
...
"#,
    ...
    let project_config_path =
        AbsolutePathBuf::try_from(workspace.path().join(".wsai-code/config.toml"))?;
    assert_eq!(
        data,
        vec![
            HooksListEntry {
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    write_project_hook_config(&repo_root.join(".wsai-code"), "echo root hook")?;
    write_project_hook_config(&worktree_root.join(".wsai-code"), "echo worktree hook")?;
    ...
    let repo_config_path =
        AbsolutePathBuf::from_absolute_path(repo_root.join(".wsai-code/config.toml"))?;
