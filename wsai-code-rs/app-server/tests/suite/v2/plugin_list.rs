...
const TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS: &str =
    "WSAI_CODE_TEST_ALLOW_HTTP_REMOTE_PLUGIN_BUNDLE_DOWNLOADS";
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.wsai-code-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.wsai-code-plugin/plugin.json"),
...
async fn plugin_list_reuses_cached_workspace_codex_plugins_setting() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(repo_root.path().join("demo-plugin/.wsai-code-plugin"))?;
    write_plugins_enabled_config_with_base_url(
        codex_home.path(),
        &format!("{}/backend-api/", server.uri()),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        repo_root
            .path()
            .join("demo-plugin/.wsai-code-plugin/plugin.json"),
        r#"{"name":"demo-plugin"}"#,
    )?;
...
async fn plugin_list_returns_share_context_for_shared_local_plugin() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(plugin_root.join(".wsai-code-plugin"))?;
    write_plugins_enabled_config(codex_home.path())?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  ...
  "plugins": [
    ...
    std::fs::write(
        plugin_root.join(".wsai-code-plugin/plugin.json"),
...
"#,
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".wsai-code"))?;
    std::fs::write(
        workspace_enabled.path().join(".wsai-code/config.toml"),
...
async fn plugin_list_returns_installed_git_source_interface_from_cache() -> Result<()> {
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(cached_plugin_root.join(".wsai-code-plugin"))?;
    std::fs::write(
        cached_plugin_root.join(".wsai-code-plugin/plugin.json"),
        r##"{
  "name": "toolkit",
  "interface": {
    "displayName": "Toolkit",
...
async fn app_server_startup_sync_downloads_remote_installed_plugin_bundles() -> Result<()> {
    ...
    wait_for_path_exists(&installed_path.join(".wsai-code-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(installed_path.join(".wsai-code-plugin/plugin.json"))?,
    )?;
    assert_eq!(
...
async fn plugin_list_sync_upgrades_and_removes_remote_installed_plugin_bundles() -> Result<()> {
    ...
    wait_for_path_exists(&new_path.join(".wsai-code-plugin/plugin.json")).await?;
    let installed_plugin_manifest: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(new_path.join(".wsai-code-plugin/plugin.json"))?,
    )?;
    assert_eq!(
        installed_plugin_manifest["version"],
        serde_json::json!("1.2.3")
...
"#,
    ...
    wait_for_path_exists(
        &codex_home.path().join(
            "plugins/cache/created-by-me-remote/private-linear/1.2.3/.wsai-code-plugin/plugin.json",
        ),
    )
    .await?;
...
"#,
    ...
    let installed_path = codex_home
        .path()
        .join("plugins/cache/openai-curated-remote/linear/1.2.3/.wsai-code-plugin/plugin.json");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.wsai-code-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
