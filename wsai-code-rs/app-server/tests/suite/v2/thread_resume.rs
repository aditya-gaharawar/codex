...
const WSAI_CODE_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Codex, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_without_managed_config_with_env(
        &codex_home_path,
        &[("WSAI_CODE_SQLITE_HOME", Some(sqlite_home))],
    )
    .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(WSAI_CODE_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
