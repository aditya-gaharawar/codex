...
use codex_exec_server::WSAI_CODE_EXEC_SERVER_URL_ENV_VAR;
...
async fn process_spawn_returns_before_exit_and_emits_exit_notification() -> Result<()> {
    ...
    let command = if cfg!(windows) {
        vec![
            ...
            "-Command".to_string(),
            concat!(
                "[IO.File]::WriteAllText($env:WSAI_CODE_PROCESS_EXEC_PROBE_FILE, 'process'); ",
                "while (!(Test-Path -LiteralPath $env:WSAI_CODE_PROCESS_EXEC_RELEASE_FILE)) { ",
        ...
        vec![
            ...
            "-c".to_string(),
            concat!(
                "printf process > \"$WSAI_CODE_PROCESS_EXEC_PROBE_FILE\"; ",
                "while [ ! -e \"$WSAI_CODE_PROCESS_EXEC_RELEASE_FILE\" ]; do sleep 0.05; done; ",
                "printf process-out; ",
    ...
    let env = HashMap::from([
        (
            "WSAI_CODE_PROCESS_EXEC_PROBE_FILE".to_string(),
            Some(probe_file.display().to_string()),
        ),
        (
            "WSAI_CODE_PROCESS_EXEC_RELEASE_FILE".to_string(),
...
async fn process_spawn_returns_error_when_local_environment_is_disabled() -> Result<()> {
    ...
    let mut mcp = TestAppServer::new_with_env(
        codex_home.path(),
        &[(WSAI_CODE_EXEC_SERVER_URL_ENV_VAR, Some("none"))],
    )
    .await?;
