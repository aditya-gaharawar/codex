...
fn fixture_paths() -> (TempDir, PathBuf, PathBuf) {
    ...
    let codex_home = root.path().join(".wsai-code");
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root
                        ...
                        .join("settings.json")
                        .display(),
                    repo_root.join(".wsai-code").join("config.toml").display()
                ),
                cwd: Some(repo_root.clone()),
                details: None,
            },
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".wsai-code").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".wsai-code").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
async fn import_repo_migrates_mcp_hooks_commands_and_subagents() {
    ...
    let config: TomlValue = toml::from_str(
        &fs::read_to_string(repo_root.join(".wsai-code").join("config.toml")).expect("read config"),
    )
    .expect("parse config");
...
"#,
    ...
    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".wsai-code").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
...
async fn import_repo_mcp_preserves_existing_same_named_server() {
    ...
    .expect("write mcp");
    fs::create_dir_all(repo_root.join(".wsai-code")).expect("create codex dir");
    let existing_config = r#"[mcp_servers.mixedTransport]
url = "https://example.com/mixed-transport"
"#;
    fs::write(
        repo_root.join(".wsai-code").join("config.toml"),
        existing_config,
    )
    .expect("write config");
...
async fn import_local_plugins_returns_completed_status() {
    ...
    fs::create_dir_all(plugin_root.join(".wsai-code-plugin")).expect("create plugin manifest dir");
    fs::create_dir_all(&codex_home).expect("create codex home");
    ...
    fs::write(
        plugin_root.join(".wsai-code-plugin").join("plugin.json"),
...
async fn import_repo_mcp_uses_home_settings_toggles_when_repo_settings_missing() {
    ...
    let outcome = service_for_paths(external_agent_home, root.path().join(".wsai-code"))
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::McpServerConfig,
            description: String::new(),
            cwd: Some(repo_root.clone()),
...
async fn import_repo_mcp_uses_local_settings_toggles_over_project_settings() {
    ...
    service_for_paths(external_agent_home, root.path().join(".wsai-code"))
        .import(vec![ExternalAgentConfigMigrationItem {
            item_type: ExternalAgentConfigMigrationItemType::McpServerConfig,
            description: String::new(),
            cwd: Some(repo_root.clone()),
...
async fn detect_repo_does_not_skip_plugins_only_configured_in_project_codex() {
    ...
    fs::create_dir_all(repo_root.join(".git")).expect("create git dir");
    fs::create_dir_all(repo_root.join(EXTERNAL_AGENT_DIR)).expect("create repo external agent dir");
    fs::create_dir_all(repo_root.join(".wsai-code")).expect("create repo codex dir");
    fs::create_dir_all(&codex_home).expect("create codex home");
    fs::write(
        repo_root.join(EXTERNAL_AGENT_DIR).join("settings.json"),
        r#"{
