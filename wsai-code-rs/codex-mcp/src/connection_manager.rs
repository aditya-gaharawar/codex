...
impl McpConnectionManager {
    ...
    pub async fn new(
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            .await;
            let codex_apps_tools_cache_context = if server_name == WSAI_CODE_APPS_MCP_SERVER_NAME {
            ...
            let runtime_auth_provider =
                if server_name == WSAI_CODE_APPS_MCP_SERVER_NAME && !uses_env_bearer_token {
    ...
    pub fn is_host_owned_codex_apps_server(&self, server_name: &str) -> bool {
        self.host_owned_codex_apps_enabled && server_name == WSAI_CODE_APPS_MCP_SERVER_NAME
    }
    ...
    pub async fn hard_refresh_codex_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let managed_client = self
            ...
            .ok_or_else(|| anyhow!("unknown MCP server '{WSAI_CODE_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{WSAI_CODE_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
