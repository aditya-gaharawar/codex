...
pub const WSAI_CODE_APPS_MCP_SERVER_NAME: &str = "codex_apps";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const WSAI_CODE_CONNECTORS_TOKEN_ENV_VAR: &str = "WSAI_CODE_CONNECTORS_TOKEN";
...
pub fn effective_mcp_servers_from_configured(
    ...
    if !host_owned_codex_apps_enabled(config, auth) {
        servers.remove(WSAI_CODE_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
fn codex_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(WSAI_CODE_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(WSAI_CODE_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(WSAI_CODE_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}
