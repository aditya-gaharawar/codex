use super::*;
use crate::codex_apps::WSAI_CODE_APPS_TOOLS_CACHE_SCHEMA_VERSION;
...
fn tool_with_model_visible_input_schema_masks_file_params() {
    let mut tool = create_test_tool(WSAI_CODE_APPS_MCP_SERVER_NAME, "upload").tool;
...
fn codex_apps_tools_cache_is_overwritten_by_last_write() {
    ...
    let tools_gateway_1 = vec![create_test_tool(WSAI_CODE_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(WSAI_CODE_APPS_MCP_SERVER_NAME, "two")];
...
fn codex_apps_tools_cache_is_scoped_per_user() {
    ...
    let tools_user_1 = vec![create_test_tool(WSAI_CODE_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(WSAI_CODE_APPS_MCP_SERVER_NAME, "two")];
...
fn codex_apps_tools_cache_preserves_formerly_disallowed_connectors() {
    ...
    let tools = vec![
        create_test_tool_with_connector(
            WSAI_CODE_APPS_MCP_SERVER_NAME,
...
fn codex_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": WSAI_CODE_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(WSAI_CODE_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
...
fn startup_cached_codex_apps_tools_loads_from_disk_cache() {
    ...
    assert_eq!(startup_tools[0].server_name, WSAI_CODE_APPS_MCP_SERVER_NAME);
...
fn startup_cached_codex_apps_tools_loads_without_server_info_cache() {
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": WSAI_CODE_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        "tools": [create_test_tool(WSAI_CODE_APPS_MCP_SERVER_NAME, "calendar_search")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");
...
async fn list_all_tools_uses_cached_tool_info_snapshot_while_client_is_pending() {
    ...
    manager.clients.insert(
        WSAI_CODE_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
    ...
    assert_eq!(tool.server_name, WSAI_CODE_APPS_MCP_SERVER_NAME);
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    assert_eq!(
        server_infos.get(WSAI_CODE_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
async fn list_all_tools_uses_cached_tool_info_snapshot_when_client_startup_fails() {
    ...
    assert_eq!(
        manager
            ...
            .await
            .get(WSAI_CODE_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
fn mcp_init_error_display_prompts_for_github_pat() {
    ...
    let expected = format!(
        "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = WSAI_CODE_GITHUB_PERSONAL_ACCESS_TOKEN"
    );
