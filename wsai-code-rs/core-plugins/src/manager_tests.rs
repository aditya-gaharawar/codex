...
fn write_plugin_with_version(
    ...
    fs::create_dir_all(plugin_root.join(".wsai-code-plugin")).unwrap();
...
async fn effective_apps_dedupes_connector_ids_across_plugins() {
    ...
    write_file(
        &plugin_a_root.join(".wsai-code-plugin/plugin.json"),
        r#"{"name":"plugin-a"}"#,
    );
    write_file(
  ...
  "apps": {
    ...
    write_file(
        &plugin_b_root.join(".wsai-code-plugin/plugin.json"),
        r#"{"name":"plugin-b"}"#,
    );
    write_file(
...
async fn install_plugin_writes_marketplace_manifest_fallback_when_missing_plugin_json() {
  ...
  "plugins": [
    ...
    assert!(!plugin_root.join(".wsai-code-plugin/plugin.json").exists());
    assert!(
        !tmp.path()
    ...
    let fallback_json: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(installed_path.join(".wsai-code-plugin/plugin.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(
...
async fn install_plugin_supports_git_subdir_marketplace_sources() {
  ...
  "plugins": [
    ...
    assert!(installed_path.join(".wsai-code-plugin/plugin.json").is_file());
}
...
async fn read_plugin_for_config_filters_mcp_servers_for_codex_backend_auth() {
  ...
  "plugins": [
    ...
    write_file(
        &repo_root.join("sample-plugin/.wsai-code-plugin/plugin.json"),
...
async fn read_plugin_for_config_does_not_fallback_from_invalid_plugin_manifest() {
  ...
  "plugins": [
    ...
    write_file(&plugin_root.join(".wsai-code-plugin/plugin.json"), "{");
    write_file(
        &tmp.path().join(CONFIG_TOML_FILE),
        r#"[features]
plugins = true
...
"#,
    ...
    write_file(
        &repo_root.join(".wsai-code/config.toml"),
        r#"[[skills.config]]
...
async fn configured_marketplace_upgrade_invalidates_cached_tool_suggest_metadata() {
  ...
  "plugins": [
    ...
    write_file(
        &remote_repo.join("plugins/sample/.wsai-code-plugin/plugin.json"),
...
async fn load_plugins_ignores_project_config_files() {
    ...
    write_file(
        &project_root.join(".wsai-code/config.toml"),
        &plugin_config_toml(/*enabled*/ true, /*plugins_feature_enabled*/ true),
    );
    ...
    let stack = ConfigLayerStack::new(
        vec![ConfigLayerEntry::new(
            ConfigLayerSource::Project {
                dot_codex_folder: AbsolutePathBuf::try_from(project_root.join(".wsai-code")).unwrap(),
            },
