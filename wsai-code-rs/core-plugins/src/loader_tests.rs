...
async fn hooks_only_scope_shares_plugin_resolution_without_loading_other_capabilities() {
  ...
  "hooks": {
    ...
    write_file(
        &disabled_root.join(".wsai-code-plugin/plugin.json"),
        r#"{"name":"disabled"}"#,
    );
    write_file(
    ...
    write_file(
        &malformed_root.join(".wsai-code-plugin/plugin.json"),
        "not valid json",
    );

    let warning_root = temp_dir.path().join("plugins/cache/test/warning/local");
    write_file(
        &warning_root.join(".wsai-code-plugin/plugin.json"),
        r#"{"name":"warning"}"#,
    );
...
fn plugin_root() -> (tempfile::TempDir, AbsolutePathBuf) {
    ...
    fs::create_dir_all(plugin_root.join(".wsai-code-plugin")).expect("create manifest dir");
...
fn write_manifest(plugin_root: &AbsolutePathBuf, manifest: &str) {
    fs::write(plugin_root.join(".wsai-code-plugin/plugin.json"), manifest).expect("write manifest");
}
