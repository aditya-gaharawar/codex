...
mod tests {
    ...
    fn install_preserves_non_global_bundle_manifest_metadata() {
        ...
        let installed_manifest: JsonValue = serde_json::from_str(
            &std::fs::read_to_string(
                result
                    .installed_path
                    .join(".wsai-code-plugin/plugin.json")
                    .as_path(),
            )
    ...
    fn find_extracted_plugin_root_uses_local_manifest_discovery() {
        ...
        std::fs::create_dir_all(extraction_root.path().join(".wsai-code-plugin"))
            .expect("create manifest dir");
        std::fs::write(
            extraction_root.path().join(".wsai-code-plugin/plugin.json"),
            r#"{"name":"linear"}"#,
        )
    ...
    fn find_extracted_plugin_root_rejects_nested_plugin_root() {
        ...
        std::fs::create_dir_all(plugin_root.join(".wsai-code-plugin")).expect("create manifest dir");
        std::fs::write(
