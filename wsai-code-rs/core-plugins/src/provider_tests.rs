...
async fn plugin_root_resolution_uses_supplied_executor_file_system() {
    ...
    let manifest_path = plugin_root.join(".wsai-code-plugin/plugin.json");
...
async fn unavailable_environment_does_not_fall_back_to_host_filesystem() {
    ...
    write_manifest(&plugin_root, ".wsai-code-plugin/plugin.json", MANIFEST_CONTENTS);
    let provider =
...
async fn malformed_preferred_manifest_does_not_fall_through_to_alternate() {
    ...
    write_manifest(&plugin_root, ".wsai-code-plugin/plugin.json", "{not-json");
    write_manifest(
        &plugin_root,
    ...
    let expected_path =
        AbsolutePathBuf::from_absolute_path_checked(plugin_root.join(".wsai-code-plugin/plugin.json"))
