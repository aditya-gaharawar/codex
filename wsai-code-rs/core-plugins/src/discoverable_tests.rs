...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(codex_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.wsai-code-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
async fn normalizes_description() {
    ...
    write_openai_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.wsai-code-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["gmail", "openai-developers"] {
        write_file(
            &curated_root.join(format!("plugins/{plugin_name}/.wsai-code-plugin/plugin.json")),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(logs.matches("gmail/.wsai-code-plugin/plugin.json").count(), 4);
    assert_eq!(
        logs.matches("openai-developers/.wsai-code-plugin/plugin.json")
            .count(),
        4
    );
}
