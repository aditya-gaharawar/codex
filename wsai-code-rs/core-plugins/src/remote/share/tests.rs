...
fn write_test_plugin(root: &Path, plugin_name: &str) -> PathBuf {
    ...
    write_file(
        &plugin_path.join(".wsai-code-plugin/plugin.json"),
...
async fn save_remote_plugin_share_creates_workspace_plugin() {
    ...
    assert_eq!(
        archive_files
            .get(".wsai-code-plugin/plugin.json")
            .map(Vec::as_slice),
...
fn archive_plugin_for_upload_places_manifest_at_archive_root() {
    ...
    assert_eq!(
        ...
        vec![
            ".wsai-code-plugin/plugin.json".to_string(),
...
fn archive_plugin_for_upload_round_trips_through_plugin_bundle_archive_with_long_paths() {
    ...
    assert_eq!(
        fs::read_to_string(destination.path().join(".wsai-code-plugin/plugin.json")).unwrap(),
        r#"{"name":"demo-plugin"}"#
    );
    assert_eq!(
