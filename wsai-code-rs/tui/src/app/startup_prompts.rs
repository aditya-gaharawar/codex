...
pub(super) fn migration_prompt_hidden(config: &Config, migration_config_key: &str) -> bool {
    match migration_config_key {
        HIDE_GPT_5_1_WSAI_CODE_MAX_MIGRATION_PROMPT_CONFIG => config
            .notices
...
mod tests {
    ...
    fn skill_load_warning_state_suppresses_repeated_active_errors() {
        ...
        let error = skill_error("/repo/.wsai-code/skills/abc/SKILL.md", "invalid description");
    ...
    fn skill_load_warning_state_displays_new_message_for_active_path() {
        let mut state = SkillLoadWarningState::default();
        let initial = skill_error("/repo/.wsai-code/skills/abc/SKILL.md", "invalid description");
        let changed = skill_error("/repo/.wsai-code/skills/abc/SKILL.md", "invalid frontmatter");
...
⚠ /repo/.wsai-code/skills/abc/SKILL.md: invalid description
");
    }
}
