...
const BASE_CLI_BUG_ISSUE_URL: &str =
    "https://github.com/wsai/wsai-code/issues/new?template=3-cli.yml";
/// Internal routing link for employee feedback follow-ups. This must not be shown to external users.
const WSAI_CODE_FEEDBACK_INTERNAL_URL: &str = "http://go/codex-feedback-internal";
...
fn slack_feedback_url(_thread_id: &str) -> String {
    WSAI_CODE_FEEDBACK_INTERNAL_URL.to_string()
}
...
mod tests {
    ...
    fn issue_url_available_for_bug_bad_result_safety_check_and_other() {
        ...
        let expected_external_url = "https://github.com/wsai/wsai-code/issues/new?template=3-cli.yml&steps=Uploaded%20thread:%20t";
