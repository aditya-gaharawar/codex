...
const MCP_WSAI_CODE_APPS_SERVER_NAME: &str = "codex_apps";
const MCP_TOOL_WSAI_CODE_APPS_META_KEY: &str = "_codex_apps";
...
impl AppLinkViewParams {
    pub(crate) fn from_url_app_server_request(
        ...
        if server_name == MCP_WSAI_CODE_APPS_SERVER_NAME {
    ...
    fn from_codex_apps_auth_url_parts(
        ...
        let auth_failure = meta?
            .as_object()?
            .get(MCP_TOOL_WSAI_CODE_APPS_META_KEY)?
            .as_object()?
...
impl AppLinkView {
    ...
    fn complete_external_flow_and_close(&mut self) {
        let should_refresh_connectors = self
            .elicitation_target
            .as_ref()
            .is_none_or(|target| target.server_name == MCP_WSAI_CODE_APPS_SERVER_NAME);
    ...
    fn install_confirmation_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let is_codex_apps_auth = is_auth_suggestion
            && self
                .elicitation_target
                .as_ref()
                .is_some_and(|target| target.server_name == MCP_WSAI_CODE_APPS_SERVER_NAME);
        lines.push(Line::from(
