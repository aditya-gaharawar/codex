...
mod tests {
    ...
    async fn open_pull_request_uses_current_branch_view_first() {
        let runner = FakeRunner::new(vec![response(
            ...
            /*exit_code*/ 0,
            r#"{"number":20252,"url":"https://github.com/wsai/wsai-code/pull/20252","state":"OPEN"}"#,
        )]);
        ...
        assert_eq!(
            ...
            StatusLinePullRequest {
                number: 20_252,
                url: "https://github.com/wsai/wsai-code/pull/20252".to_string(),
            }
        );
    ...
    async fn open_pull_request_falls_back_to_parent_repo_commit_lookup() {
        let runner = FakeRunner::new(vec![
            ...
            response(
                ...
                /*exit_code*/ 0,
                r#"[{"number":20252,"html_url":"https://github.com/wsai/wsai-code/pull/20252","state":"open"}]"#,
            ),
        ]);
    ...
    fn status_line_pr_view_parser_requires_open_pr() {
        ...
        assert_eq!(
            pull_request_from_view_output(
                r#"{"number":20252,"url":"https://github.com/wsai/wsai-code/pull/20252","state":"MERGED"}"#
            ),
            None
        );
    }
