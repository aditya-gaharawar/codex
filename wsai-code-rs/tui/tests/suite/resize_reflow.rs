...
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("WSAI_CODE_HOME={}", codex_home.path().display()))
...
fn codex_binary(repo_root: &Path) -> Result<PathBuf> {
    ...
    let fallback = repo_root.join("wsai-code-rs/target/debug/codex");
    anyhow::ensure!(
        fallback.is_file(),
        "codex binary is unavailable; run `cargo build -p wsai-code-cli` first"
    );
    Ok(fallback)
}
