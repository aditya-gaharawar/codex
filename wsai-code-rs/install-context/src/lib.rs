...
pub enum InstallMethod {
    Standalone {
        ...
        /// such as
        /// `~/.wsai-code/packages/standalone/releases/0.111.0-x86_64-unknown-linux-musl`.
...
impl InstallContext {
    ...
    pub fn current() -> &'static Self {
        INSTALL_CONTEXT.get_or_init(|| {
            ...
            let managed_by_npm = std::env::var_os("WSAI_CODE_MANAGED_BY_NPM").is_some();
            let managed_by_bun = std::env::var_os("WSAI_CODE_MANAGED_BY_BUN").is_some();
            Self::from_exe(
                cfg!(target_os = "macos"),
...
mod tests {
    ...
    fn detects_standalone_install_from_release_layout() -> std::io::Result<()> {
        ...
        let exe_path = release_dir.join(if cfg!(windows) { "wsai-code.exe" } else { "wsai-code" });
        fs::write(&exe_path, "")?;
    ...
    fn detects_package_layout_independently_from_install_method() -> std::io::Result<()> {
        ...
        let exe_path = bin_dir.join(if cfg!(windows) { "wsai-code.exe" } else { "wsai-code" });
        fs::write(&exe_path, "")?;
        fs::write(resources_dir.join(TEST_RESOURCE_NAME), "")?;
