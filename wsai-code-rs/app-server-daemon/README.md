...
It is intended for WSAI CODE instances launched over SSH, including fresh developer
...
settings under `WSAI_CODE_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes WSAI CODE is installed through `install.sh` and always launches
the standalone managed binary under `WSAI_CODE_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `WSAI_CODE_HOME/packages/standalone/current/codex` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `WSAI_CODE_HOME/packages/standalone/current/codex` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
All mutating lifecycle commands are serialized per `WSAI_CODE_HOME`, so a concurrent
...
The daemon stores its local state under `WSAI_CODE_HOME/app-server-daemon/`:
