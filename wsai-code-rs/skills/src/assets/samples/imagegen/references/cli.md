...
Set a stable path to the skill CLI (default `WSAI_CODE_HOME` is `~/.wsai-code`):

```
export WSAI_CODE_HOME="${WSAI_CODE_HOME:-$HOME/.codex}"
export IMAGE_GEN="$WSAI_CODE_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$WSAI_CODE_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
