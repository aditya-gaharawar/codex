...
- the user wants WSAI CODE to see the updated plugin without manually editing marketplace files
...
1. Update the plugin manifest to a single WSAI CODE cachebuster suffix:
...
outside WSAI CODE depends on a specific token:
...
6. Prompt the user to use a new thread to try the updated plugin, so that WSAI CODE picks up new skills
   and tools.
...
Replace the existing WSAI CODE cachebuster instead of appending another one. Do not keep incrementing
...
- Do not tell the user to run `codex plugin marketplace add` for the default personal-marketplace
  flow. That marketplace is discovered implicitly by WSAI CODE.
