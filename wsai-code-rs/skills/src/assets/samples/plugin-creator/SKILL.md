---
name: plugin-creator
description: Create and scaffold plugin directories for WSAI CODE with a required `.codex-plugin/plugin.json`, optional plugin folders/files, valid manifest defaults, and personal-marketplace entries by default. Use when WSAI CODE needs to create a new personal plugin, add optional plugin structure, generate or update marketplace entries for plugin ordering and availability metadata, or update an existing local plugin during development with the CLI-driven cachebuster and reinstall flow.
---
...
3. Generate or update the personal marketplace entry when the plugin should appear in WSAI CODE UI ordering:
...
- Treat plugin order in `plugins[]` as render order in WSAI CODE. Append new entries unless a user explicitly asks to reorder the list.
...
- If WSAI CODE would need approval to write the marketplace file, ask for that approval before
...
- When the workflow created or updated a marketplace-backed plugin, end the final user-facing
  response with a short WSAI CODE app handoff. Say `To view this in the WSAI CODE app:` and write
...
- Do not add `pluginName` or `hostId` query parameters to these deeplinks. WSAI CODE derives both after
