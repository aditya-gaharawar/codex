...
The builder creates a canonical WSAI CODE package directory:
...
host platform. On Linux, that default is a musl target to match WSAI CODE release
...
read from `[workspace.package].version` in `wsai-code-rs/Cargo.toml`.
...
downloads and verifies the matching WSAI CODE-built V8 release pair before invoking
